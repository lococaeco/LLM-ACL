...
class ModelEvaluator:
    ...
    def evaluate_model_and_save(self, model_path: Path):
        ...
        print(f"환경: {env_name}, 알고리즘: {algorithm}")
        ...
        if config:
            ...
            config_path = f"wandb/run-{model_path.parent.name}/files/config.yaml"
        else:
        ...
        try:
            ...
            # 정상 평가
            print(f"정상 환경 평가 중...")
            normal_mean, normal_std = self.evaluate_normal(model, env_name, n_episodes=5)
            print(f"  정상 평균 보상: {normal_mean:.2f} ± {normal_std:.2f}")
            
            # 강건성 평가
            print(f"강건성 테스트 중...")
            robust_results = self.evaluate_robust(model, env_name, n_episodes=5)
            print(f"  강건성 평균 보상: {robust_results['mean']:.2f} ± {robust_results['std']:.2f}")
