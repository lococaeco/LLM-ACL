...
class LLMLoader:
    # __init__에 quantization 관련 인자를 받을 수 있도록 수정
    def __init__(self, model_id: str, tensor_parallel_size: int = 1, gpu_memory_utilization: float = 0.80, quantization: str = None):
        self.model_id = model_id
        self.tensor_parallel_size = tensor_parallel_size
        self.gpu_memory_utilization = gpu_memory_utilization
        self.quantization = quantization # 예: "fp8", "awq", "gptq" 등
        self.llm = None
        self.tokenizer = None
    ...
    def load(self):
        ...
        self.llm = LLM(
            ...
            quantization=self.quantization, # 양자화 설정 전달
            enforce_eager=True # 경우에 따라 CUDA graph 에러 방지용
        )
        
        self.tokenizer = self.llm.get_tokenizer()
        # vLLM 객체는 model 속성이 없으므로 self.llm 자체를 반환하거나 구조를 맞춰야 함
        # Decider에서 self.model.generate를 호출한다면 vLLM 방식과는 다르므로 주의 필요
